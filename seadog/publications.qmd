...
toc: false
bibliography: publications_eric_pante.bib
nocite: |
  @*
csl: apa.csl
execute:
...
get_dataset_info <- function(entry) {
  dat <- entry$dataset_info %||% NA_character_
  if (isTRUE(is.na(dat)) || !nzchar(dat)) NA_character_ else dat
}
...
bib  <- ReadBib("publications_eric_pante.bib", check = FALSE)
keys <- names(bib)

extra_links <- tibble(key = character(), preprint = character(), github = character(), recommendation = character())
...
rows <- tibble(
  ...
  mutate(
    ...
    dataset_info = map_chr(entry, get_dataset_info),
    ...
    github_raw = map_chr(entry, ~ { v <- .x$github; if (is.null(v) || length(v)==0) NA_character_ else as.character(v[[1]]) }),
    recommendation_raw = map_chr(entry, ~ { v <- .x$recommendation; if (is.null(v) || length(v)==0) NA_character_ else as.character(v[[1]]) }),
    
    has_preprint_key  = map_lgl(entry, ~ !is.null(.x$preprint)  && length(.x$preprint)  > 0),
    has_github_key = map_lgl(entry, ~ !is.null(.x$github) && length(.x$github) > 0),
    has_recommendation_key = map_lgl(entry, ~ !is.null(.x$recommendation) && length(.x$recommendation) > 0)
  ) |>
  left_join(extra_links, by = "key") |>
  mutate(
    preprint_val  = coalesce(preprint,  preprint_raw),
    github_val = coalesce(github, github_raw),
    recommendation_val = coalesce(recommendation, recommendation_raw)
  ) |>
  select(-any_of(c("preprint", "github","recommendation"))) |>
  mutate(
    preprint_url  = normalize_link(preprint_val),
    github_url = normalize_link(github_val),
    recommendation_url = normalize_link(recommendation_val)
  ) |>
  mutate(
...
(function() {
  ...
  document.addEventListener('click', function(evt) {
    ...
    if (action === 'abstract' || action === 'dataset_info' || action === 'citation' || action === 'bib') {
     ...
     if (action === 'dataset_info') {
        const payloadEl = card.querySelector('.payload-dataset_info');
        const text = (payloadEl?.textContent || 'No data available.').trim();
        showPanel(card, label, text, 'dataset_info');
        return;
      }
...
build_card <- function(
  ...
  abstract, bibtex,dataset_info,
  preprint_url = NA_character_, github_url = NA_character_, recommendation_url = NA_character_,
  doi_url = NA_character_, primary = FALSE, ...
) {
  has_preprint_key  <- !is.null(entry$preprint)  && length(entry$preprint)  > 0
  has_github_key <- !is.null(entry$github) && length(entry$github) > 0
  has_recommendation_key <- !is.null(entry$recommendation) && length(entry$recommendation) > 0

  payloads <- tags$div(
    style = "display:none;",
    tags$div(class = "payload-abstract", abstract %||% ""),
    tags$div(class = "payload-dataset_info", dataset_info %||% ""),
  ...
  actions <- tags$div(
    ...
    tags$a("Data",     href = "#", `data-action`="dataset_info",     role="button"),
    ...
    if (isTRUE(has_github_key))
      tags$a(
        ...
        target = "_blank", 
        rel="noopener"),
    if (isTRUE(has_recommendation_key))
      tags$a("Recommendation", class = "external", href = (recommendation_url %||% "#"), target = "_blank", rel="noopener")
  )
...
groups <- lapply(years, function(y) {
  ...
  subset_min <- subset |>
    select(
      key, entry, title, authors_card, year, venue,
      abstract, dataset_info, bibtex,
      preprint_url, github_url, recommendation_url, doi_url, primary
    )
